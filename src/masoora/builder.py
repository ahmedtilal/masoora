...
class PipelineBuilder(Generic[ContextT]):
    ...
    def with_read_step(self, fn: Callable[[ContextT], Any], *, output: str) -> Self:
        """fn(context) -> dataset, stored at catalog[output]."""
        self._steps.append(ReadStep(fn=fn, output=output))
        return self

    def with_transform_step(
        self, fn: Callable[..., Any], *, inputs: Sequence[str], output: str
    ) -> Self:
    ...
    def with_seed(self, key: str) -> Self:
    ...
    def build(self) -> Pipeline[ContextT]:
        ...
        self._validate()
        return Pipeline(steps=topo_sort(self._steps))
